...
def send_query_request(url, query, history_list=None):
    ...
    payload = {
        ...
        "enableExtraction": True,  # 否是抽取信息开关
        "extractionRules": [
            {
...
def process_excel(file_path, api_url, sample_history=None):
    ...
    try:
        ...
        df['extract_info'] = None # 用于存储抽取的信息
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df)):
            ...
            if result and result.get('success') and 'data' in result:
                ...
                extracted_info = result['data'].get('extractedInfo', [])
                ...
                # 填充抽取信息
                if extracted_info:
                    df.at[idx, 'extract_info'] = extracted_info
...
if __name__ == "__main__":
    """
    python test_query_cls.py -f D:\工作文件\multiple_intentions\query分类.xlsx -u http://10.19.16.193:8883/ai-test/yun/ai/text/query/rewrite
    
    python test_query_cls.py -f query字数抽取.xlsx -u http://10.19.16.193:8883/ai-test/yun/ai/text/query/rewrite
    """
    main()
