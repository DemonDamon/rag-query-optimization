...
from models import QueryRewriteRequest, QueryRewriteResponse, QueryRewriteResults, ExtractionRule, Message
...
def format_history_for_log(history_list: List[Message]) -> List[str]:
    """格式化历史查询列表用于日志记录，只保留content内容"""
    if not history_list:
        return []
    return [f"{msg.role}: {msg.content}" for msg in history_list]


def call_llm_for_judge(query: str, history_queries: List[Message], request_id: str = ""):
    """调用判断模型服务判断是否需要改写"""
        ...
        if history_queries:
            ...
            formatted_message(
                ...
                "历史查询格式化 - 原始数据",
                {
                    ...
                    "history_count": len(history_queries),
                    "history_roles": [msg.role for msg in history_queries],
                    "first_message": history_queries[0].dict() if history_queries else None,
                    "all_messages": [{"index": i, "role": msg.role, "content": msg.content[:30] + "..." if len(msg.content) > 30 else msg.content} for i, msg in enumerate(history_queries)]
                },
                level="debug"
            )
            
            # 简单按顺序处理，确保所有消息都被正确处理
            history_parts = []
            qa_pair_num = 0
            
            for idx, msg in enumerate(history_queries):
                formatted_message(
                    logger,
                    f"处理消息 {idx}",
                    {
                ...
                if msg.role == "user":
                    qa_pair_num += 1
                    q_text = f"Q{qa_pair_num}: {msg.content}"
                    history_parts.append(q_text)
                    ...
                    # 检查前一个消息是否是user，如果是则使用相同的编号，否则新增编号
                    if idx > 0 and history_queries[idx - 1].role == "user":
                        # 这是对前一个user问题的回答
                        a_text = f"A{qa_pair_num}: {msg.content}"
                    else:
                        # 这是单独的assistant消息
                        qa_pair_num += 1
                        a_text = f"A{qa_pair_num}: {msg.content}"
                    
                    history_parts.append(a_text)
...
def call_llm_for_rewrite(query: str, history_queries: List[Message], request_id: str = ""):
    """调用改写模型服务进行查询改写"""
    try:
        global prompts
...
def call_judge_and_rewrite_pipeline(query: str, history_queries: List[Message], request_id: str = ""):
...
async def rewrite_query(request: QueryRewriteRequest):
    ...
    formatted_message(
        ...
        f"接收请求",
        {
            ...
            "query": request.query,
            "historyList": format_history_for_log(request.historyList) if request.historyList else [],
